...
3. 若 `Week.usedPercent >= 100`，不调用模型，安排在周额度 `resetAt + 5 分钟`复查。如果刷新后仍显示耗尽，则每 5 分钟复查一次。
4. 周额度恢复后，再判断 5 小时窗口。当 `5h.usedPercent` 为 0，且 `resetAt` 几乎等于“当前时间 + 5 小时”时，将其视为未启动的浮动窗口并立即 anchor。
5. 若 5 小时窗口仍在真正计时，则在其 `resetAt + 5 分钟`创建一次性检查；成功 anchor 后继续自调度。
6. 平常在 00:00–05:59 不启动新窗口。周额度恢复检查是例外：即使发生在夜间，也会在确认两种额度都可用后尝试对齐。
...
3. If `Week.usedPercent >= 100`, it schedules a check at the weekly `resetAt + 5 minutes` and never calls the model. If the quota still reports exhausted then, it retries every five minutes.
4. When weekly quota is available, `5h.usedPercent === 0` plus a reset time approximately five hours ahead is treated as an unused, floating window and anchored immediately.
5. A genuinely active 5-hour window is checked again at `resetAt + 5 minutes`; after a successful anchor the controller continues self-scheduling.
6. Ordinary checks avoid starting a window between 00:00 and 05:59. Weekly recovery checks may anchor during these hours once both quotas are available.
